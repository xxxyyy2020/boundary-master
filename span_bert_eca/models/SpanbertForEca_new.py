...
from .layers.pointer_network import PointerNetwork, PointerNetwork_Multi
...
from process_data.eca_seq import bert_extract_item
...
class Bert2Gru(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, context_mask = None, labels=None, span_labels = None, start_positions=None,end_positions=None, testing = False):
        ...
        if labels != None:
            ...
            target, _ = torch.nn.utils.rnn.pad_packed_sequence(labels, total_length=max_len)
        encoder_outputs, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)

        cls_out = encoder_outputs[:,0,:] 
