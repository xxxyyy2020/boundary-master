...
from process_data.eca_seq import eca_processors as processors, batch_generator, bert_extract_item, extract_multi_item 
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    features = convert_examples_to_features(examples=examples,
                                            ...
                                            max_seq_length=args.max_seq_length, #需要测试一下两部分数据的最大长度是什么
...
def main():
    ...
    for i in range(args.split_times):
        ...
        print('\n result of all ：bert_crf_results_{} = {}\n'.format(args.data_type, metrics))


        if os.path.exists(args.data_dir):
            shutil.rmtree(args.data_dir)
            print('remove file:', args.data_dir)
