...
def get_prf(pre_label_l, tru_label_l, examples):
    ...
    """ 
    # print('pre_label_l = ', pre_label_l)
    # print('tru_label_l = ', tru_label_l)
    ...
    for pre_label, tru_label, data_len, exam_label in zip(pre_label_l, tru_label_l, data_len_c, tru_example_label):
        ...
        pre_ph, tru_ph, corr_ph = 0, 0, 0
        span_l_t = get_span(tru_label)
        span_l_p = get_span(pre_label)
        # print('tru_label = ', tru_label)
        # print('span_l_t = ', span_l_t)
        ...
        p += 1.0 * corr_ph / pre_ph if pre_ph > 0 else 0
        r += 1.0 * corr_ph / tru_ph if tru_ph > 0 else 0
    ...
    result = {'span_p': np.around(PS, decimals=4),'span_r': np.around(RS, decimals=4), 'span_f': np.around(FS, decimals=4)}
    result_span = "span: {} {} {}".format(np.around(PS, decimals=4), np.around(RS, decimals=4), np.around(FS, decimals=4))
    # result_word = "word: {} {} {}".format( np.around(p_p, decimals=4),  np.around(r_p, decimals=4), np.around(f, decimals=4))
    # result_phrase = "clause: {} {} {}".format(np.around(p_c, decimals=4),  np.around(r_c, decimals=4), np.around(f_c, decimals=4))
