...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, testing = None):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False) #[batch, max_len, dim]
        ...
        if start_positions is not None and end_positions is not None:
            ...
            return (start_logits.argmax(-1), end_logits.argmax(-1))
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    Example usage:
    ```python
    修改成自己计算的loss函数，解决针对多个起点和终点的计算方法
    # Already been converted into WordPiece token ids
    input_ids = torch.LongTensor([[31, 51, 99], [15, 5, 0]])
    input_mask = torch.LongTensor([[1, 1, 1], [1, 1, 0]])
    token_type_ids = torch.LongTensor([[0, 0, 1], [0, 1, 0]])
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, testing = None):
        ...
        def compute_loss(para_logits, positions):
            ...
            one_hot_positions = torch.zeros(batch_size, seq_length).cuda()
            one_hot_positions = one_hot_positions.scatter_(1, positions.unsqueeze(1), 1)
